 ...
 */
public class MergeArray_22 {

    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        int readLen = Integer.parseInt(scanner.nextLine());
        int totalArray = Integer.parseInt(scanner.nextLine());
        List<List<String>> arrays = new ArrayList<>(totalArray);
        int totalNum = 0;
        ...
        while (totalNum > 0) {
            for (int i = 0; i < arrays.size(); i++) {
                Iterator<String> iterator = arrays.get(i).iterator();
                int len = 0;
